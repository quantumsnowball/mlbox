...
def train(env_name='CartPole-v0', hidden_sizes=[32], lr=1e-2,
    ...
    def train_one_epoch():
        ...
        optimizer.zero_grad()
        batch_loss = compute_loss(obs=torch.as_tensor(np.array(batch_obs), dtype=torch.float32).to(DEVICE),
                                  act=torch.as_tensor(
                                      np.array(batch_acts), dtype=torch.int32).to(DEVICE),
                                  weights=torch.as_tensor(
                                      np.array(batch_weights), dtype=torch.float32).to(DEVICE),
                                  )
        batch_loss.backward()
        optimizer.step()
